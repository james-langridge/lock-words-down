...
const TermEntryList = () => {
  ...
  const selectedSelection = useAppSelector(
    (state) => state.selections.selectedSelection
  );
  ...
  const deleteTermEntry = async (termEntry: TermEntry) => {
    const result = window.confirm(
      `Delete word: ${termEntry.term}? You cannot undo this!`
    );
    if (result) {
      try {
  ...
  const deleteSelection = async (selectedSelection: Selection) => {
    const result = window.confirm(
      `Delete selection: ${selectedSelection.selectionTitle}? You cannot undo this!`
    );
    if (result) {
      try {
  ...
  return (
    <Container>
      ...
      <GameTitleInput changeGameTitle={changeGameTitle} gameTitle={gameTitle} />
      <ScrollingToggle toggleScrolling={toggleScrolling} />
      <br />
      ...
      <Alert variant={selectedSelection ? "success" : "danger"}>
        Word Selection:{" "}
        {selectedSelection ? selectedSelection.selectionTitle : "none selected"}
      </Alert>
      <TableButtons
